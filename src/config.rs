...
impl Config {
    pub fn validate(&self) -> Result<bool, DBRError> {
        if self.replica_id == 0 {
            return Err(DBRError::ConfigError(
                "invalid ReplicaID, it must be >= 1".to_string(),
            ));
        }
